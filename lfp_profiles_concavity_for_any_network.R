AnyNetworkProfilesConcavity <- function(xycoords, network, prefix, dem, direction,
  ...
  # Edited version from: https://github.com/geofis/rgrass/blob/master/lfp_profiles_concavity_for_any_network.R
  ...
  dfs3$stream <- factor(dfs3$stream, levels = SortMixed(levels(dfs3$stream), decreasing = T))
  dfs3 <- dfs3[order(dfs3$stream),]
